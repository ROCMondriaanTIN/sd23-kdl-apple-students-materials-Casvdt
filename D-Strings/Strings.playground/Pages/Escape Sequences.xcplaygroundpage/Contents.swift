 ...
 */
func removeEscapeCharacter(){
    let originalString = "Roses are red.\nViolets are blue."
    let modifiedString = originalString.reduce(into: "") { result, character in
        if character == "\n" {
            result.append(" ")
        } else {
    ...
    print(modifiedString)  // Output: "Roses are red. Violets are blue."
}
